...
export const validationSchemas = {
  ...
  createQuote: Joi.object({
    ...
    valid_until: Joi.date().iso().greater('now').required(),
  }),
  ...
  updateBookingStatus: Joi.object({
    status: Joi.string().valid('scheduled', 'in_progress', 'completed', 'cancelled', 'rescheduled').required(),
  }),

  // Booking notes update
  updateBookingNotes: Joi.object({
    notes: Joi.string().max(1000).optional().allow(null, ''),
  }),
  ...
  completeBooking: Joi.object({
    completion_notes: Joi.string().max(1000).optional().allow(null, ''),
    final_amount: Joi.number().positive().optional(),
  }),
};

// Individual schema exports for easier use
export const createQuoteSchema = validationSchemas.createQuote;
export const updateQuoteSchema = validationSchemas.updateQuote;
export const updateBookingStatusSchema = validationSchemas.updateBookingStatus;
export const updateBookingNotesSchema = validationSchemas.updateBookingNotes;
export const cancelBookingSchema = validationSchemas.cancelBooking;
export const completeBookingSchema = validationSchemas.completeBooking;
